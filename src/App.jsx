...
function AppContent({ usuario, page, setPage, tick, setTick }) {
  const theme = useTheme();
  const mobile = useMediaQuery(theme.breakpoints.down('md'));
    ...
    <Box sx={{ display: 'flex', minHeight: '100dvh', bgcolor: 'background.default', color: 'text.primary' }}>
      ...
      {!mobile && (
        ...
        >
          <Box sx={{ px: 1.5, py: 3 }}>
            ...
            <Typography variant="h5" sx={{ lineHeight: 1.15 }}>Nuestro sistema</Typography>
      ...
      <Box
        ...
        sx={{
          ...
          minWidth: 0,
          p: { xs: 2, md: 3 },
          pb: { xs: '76px', md: 3 }, // espacio para bottom nav en móvil
...
export default function App() {
  ...
  return (
    <ThemeProvider theme={theme}>
      <CssBaseline />
      <AppContent usuario={usuario} page={page} setPage={setPage} tick={tick} setTick={setTick} />
    </ThemeProvider>
  );
}
